...
`surface`, `radius3xl`, `elev4`, inset `s6` from the screen edges, padding `s7 s6 s6`. An optional 56pt icon disc (`brandSoft`/`brandSoftInk`, or `dangerTint`/`danger` for `danger`) sits above a `type.title` semi title and `type.bodyLg` `ink2` body. Text is `start`-aligned, not centred — centred body copy is unreadable at Sorani line lengths. Actions are **stacked and full-width**, each ≥52, ordered **the safe alternative first where one exists, then the destructive action, then `Cancel`**. All three references in `design/reference/calm/` are drawn that way — *Keep editing* above *Discard*, *Keep it — mark it sold* above *Delete* above *Cancel* — and `calm-visual-parity` rule 1 makes the reference the authority. `SPEC.md` §7's "no dialog is ever dismissed into a destructive outcome" points the same way: the action nearest the thumb should be the one that loses nothing. A dialog with no safe alternative is the two-slot case and reads confirm-then-cancel. (Corrected in EPIC-08, finding F-8.4; this file previously said "destructive first".) Entry is scale 0.96 → 1 with a fade over `motion.base` with `easeSettle`.
