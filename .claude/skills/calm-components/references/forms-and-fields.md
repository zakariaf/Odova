...
- **`CalmSwitch`** is 56×34 with a 28pt `surface` thumb on `elev1`, travelling 22pt toward the `end` edge over `motion.base` with `easeSettle`. Track off `surface3`, on `brand`. Never use it as the row's tap target: `CalmListRow.switchRow` is not navigable, the whole row toggles, and the pair is one `MergeSemantics` node labelled by the row title.
