...
- **Write these tests first** — `test/features/first_run/first_run_language_screen_test.dart`:
  ...
  - `a device language outside the six preselects System and shows the not-translated-yet line` — a `pt-BR` device gets the exact §8 sentence with `{deviceLanguage}` filled.
...
| F-9.3 | The first-run restore link is "Moving from another phone? Restore a backup" in §8 and "I already have an Odova backup" in §14. | §8 owns the screen, so §8's string wins and goes into the ARB. §14 is corrected in this PR. |
