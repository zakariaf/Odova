...
| F-9.16 | **Calm draws three controls below its own stated touch minimum, and no implementation can satisfy both.** `odova.css` declares `--touch-min: 52px` and applies it to some elements — and then draws `.chip` at `min-height: 40px`, `.segmented__opt` at 46, and `.swatch` at 26 square. `RenderCalmTapTarget` meets the floor by growing the LAYOUT box, so every one of those comes out 52 and the rows that hold them come out 12, 6 and 26 pixels taller than the reference. Measured on `firstrun.vehicle`: the app tracks the reference to within 3px down to the Fuel label, gains ~25px across the chipbar, and holds it. Seen again on `vehicle.edit` in two more places. | **Not fixed here, and not papered over.** `calm_touch_targets_test.dart`'s own comment already states the intent the code does not implement — "several Calm widgets paint smaller than they hit on purpose — a chip paints 40, a small button 42, a modal action 44". Three resolutions, and choosing is a design decision rather than an engineering one. **(a)** The target overflows the paint. It keeps both the drawing and the floor, and it does NOT work everywhere: inside `CalmChipBar`'s scroll viewport the overflow is bounded by the bar's 4pt padding, and a 26pt swatch in a 26pt row cannot receive a 52pt tap at all. **(b)** The containers grow — +4 on the chipbar, +26 on the swatch row — which changes what the design draws. **(c)** The floor drops for these three, which is a WCAG 2.5.5 decision and belongs beside `design/calm/ACCESSIBILITY-FINDING.md`. **EPIC-17 owns it.** It blocks parity on every screen carrying a chipbar, a flat segmented control or a swatch row — `log.fillup`, `reminders.edit`, `history`, `costs`, and both screens this epic has captured so far. No tolerance is widened and no reference is re-shot in the meantime. |
...
| F-9.20 | §8's normal-mode sketch draws a VIN field, two disclosure groups and a plain `Delete vehicle`; the artboard has no VIN field, no Notes field, neither disclosure, and a delete row reading "Delete Golf and its 412 entries". §8's prose then names notes, `expected_annual_m`, `is_business`, `notifications_muted`, purchase date and price — none of which the artboard draws either. | **Revised once the capture was taken: the artboard is COMPLETE, not a crop.** It ends with the Delete row and the home bar, so it is drawing the whole screen — and that screen has no VIN and no Notes. §8's prose lists both, and the schema and the backup format carry both. So the two genuinely disagree, and neither is ignorable: CLAUDE.md §7 makes the reference the authority on appearance, §3 makes the spec the authority on content. **Content wins here and the parity failure is accepted and recorded**, because the alternative is deleting a field a user's data already holds in order to make a screenshot match. VIN and Notes sit at the top level today and belong in a collapsed group; when F-9.23's disclosure contents arrive, a third group is where they go, and the reference is re-shot in that PR. The delete ROW's wording follows the artboard and takes its own ARB key — it is a row, not a dialog title, so it carries no question mark. |
...
| F-9.23 | §8's `vehicle.edit` carries two disclosure groups whose CONTENTS need components no screen in this epic has: **Purchase and sale** wants a calendar picker (`purchase_date`, ≤ today) and a money field with a currency, and **This vehicle's units & currency** wants six override controls, each offering **Automatic**. | **The six overrides belong to EPIC-14 and `vehicle.edit` composes them.** `settings.units` builds exactly the same six — `currency`, `distance_unit`, `volume_unit`, `consumption_unit`, `notice_distance_m`, `notice_days` — and building them here and again there is the duplication the epic split exists to prevent, in the same way `LanguageRowList` was extracted for EPIC-14 rather than copied. Two of the six cannot even be labelled today: `unitVolumeGallon` is the shared "gal" with the SYSTEM named elsewhere (SPEC.md §5 forbids conflating a US and an imperial gallon), and four of the six `ConsumptionUnit` values have no ARB label at all. The DATA half is already built and tested — `VehicleEditDraft` writes null for every override and a test walks all six. The date picker and the money field are likewise EPIC-11's and EPIC-14's, wanted by five `log.*` screens before this one. **Both groups are ABSENT from the screen rather than stubbed**, because a collapsed group that opens on nothing is a control that lies. |
