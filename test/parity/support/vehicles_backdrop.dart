/// The REAL `vehicles`, as the artboard draws it — the one authority for every
/// capture that needs that screen.
///
/// `dialog.confirmDelete` was shot over `vehicles`. EPIC-08 could not use the
/// real screen because it did not exist, so it hand-built `VehiclesBackdrop`
/// out of EPIC-03's widgets and recorded the swap as **F-8.2**: "EPIC-09 and
/// EPIC-10 replace them with the real screens and re-run the three captures. If
/// the parity result CHANGES when they do, this stand-in was lying." This file
/// is that replacement.
///
/// The fixture is `vehicles_parity_test.dart`'s, imported rather than re-typed:
/// two captures of one screen have to be photographing one screen.
library;
...
/// `vehicles` under the artboard's own garage, ready to be a capture's `child`.
///
/// Pass `tab: 3` alongside it: §7 opens `vehicles` from the Settings row, so
/// the reference draws the Settings tab active beneath it.
///
/// The artboard translates the vehicle NAMES and the make and model. The app
/// never would — they are the user's own words — but the capture has to compare
/// like with like, so the fixture carries the artboard's text in [rtl].
Widget vehiclesBackdrop({required bool rtl, required Locale locale}) =>
    ProviderScope(
      overrides: <Override>[
